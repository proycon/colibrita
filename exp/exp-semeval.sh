...
for LANGPAIR in $LANGPAIRS; do
    ...
    if [ ! -f train-$CORPUS/model/moses.ini ]; then
        echo "Moses failed?" >&2
        continue
    fi
    ...
    for CONF in $TRAINCONFIGURATIONS; do
        ...
        if [ ! -d "${CORPUS}-mosesbaseline" ]; then 
            ...
            colibrita --test $TESTDIR/corpus/${LANGPAIR}.gold.tokenised.xml -Z -o $CORPUS-mosesbaseline --moseslm $CORPUS-train.${L2}.lm
        fi
        ...
        if [ ! -d $CORPUS-${CONF}-moses ]; then
            ...
            colibrita --test $TESTDIR/corpus/${LANGPAIR}.gold.tokenised.xml -l $L -r $R -Z -o $CORPUS-${CONF}-moses --lm $CORPUS-train.${L2}.lm 
        fi


        if [ ! -d $CORPUS-${CONF}-mosesweighted ]; then
            ln -s $CORPUS-$CONF $CORPUS-${CONF}-mosesweighted
            colibrita --test $TESTDIR/corpus/${LANGPAIR}.gold.tokenised.xml -l $L -r $R -X -o $CORPUS-${CONF}-mosesweighted --lm $CORPUS-train.${L2}.lm 
        fi

        if [ ! -d $CORPUS-${CONF}-mosescut0.7 ]; then
            ln -s $CORPUS-$CONF $CORPUS-${CONF}-mosescut0.7
            colibrita --test $TESTDIR/corpus/${LANGPAIR}.gold.tokenised.xml -l $L -r $R -A 0.7 -o $CORPUS-${CONF}-mosescut0.7 --lm $CORPUS-train.${L2}.lm 
        fi

        if [ ! -d $CORPUS-${CONF}-mosescut0.8 ]; then
            ln -s $CORPUS-$CONF $CORPUS-${CONF}-mosescut0.8
            colibrita --test $TESTDIR/corpus/${LANGPAIR}.gold.tokenised.xml -l $L -r $R -A 0.8 -o $CORPUS-${CONF}-mosescut0.8 --lm $CORPUS-train.${L2}.lm 
        fi

        if [ ! -d $CORPUS-${CONF}-mosescut0.9 ]; then
            ln -s $CORPUS-$CONF $CORPUS-${CONF}-mosescut0.9
            colibrita --test $TESTDIR/corpus/${LANGPAIR}.gold.tokenised.xml -l $L -r $R -A 0.9 -o $CORPUS-${CONF}-mosescut0.9 --lm $CORPUS-train.${L2}.lm 
        fi
    done
